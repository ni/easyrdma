...
class BufferCompletion {
    public:
        ...
        void* GetContext() { return completionFuture.get().context; };
        bool IsCompleted() { return completionFuture.wait_for(std::chrono::milliseconds(0)) == std::future_status::ready; };
        size_t GetCompletedBytes() { return completionFuture.get().completedBytes; };
    protected:
...
class BufferRegion : public easyrdma_InternalBufferRegion {
    public:
        BufferRegion(const easyrdma_InternalBufferRegion& _region) : easyrdma_InternalBufferRegion(_region) {
        }
        BufferRegion() : easyrdma_InternalBufferRegion({}) {
        }
...
class Session {
    public:
        Session() : session(easyrdma_InvalidSession) {
        }
        Session(easyrdma_Session _session) : session(_session) {
        }
