...
class RdmaAddress {
    public:
        RdmaAddress() : address( { 0 } ) {
        };
        RdmaAddress(const std::string& _address, uint16_t port) : address( { 0 } ) {
        ...
        RdmaAddress(const sockaddr* _address) : address( { 0 } ) {
            memcpy(&address, _address, GetAddrSize(_address));
        }
        RdmaAddress(const sockaddr_in& _address) : address( { 0 } ){
