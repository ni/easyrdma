...
{
public:
    RdmaError() : errorCode(0), errorSubCode(0), filename(nullptr), fileLineNumber(0) {
    };
...
{
public:
    RdmaException(const RdmaError& _rdmaError) : rdmaError(_rdmaError) {
    };
    ...
    std::string GetExtendedErrorInfo() const {
        std::stringstream ss;
        ss << "RdmaException: " << "ErrorCode: " << rdmaError.GetCode() << " ErrorSubCode: " << rdmaError.errorSubCode << " File: " << rdmaError.filename << ":" << rdmaError.fileLineNumber;
        return ss.str();
    };
    RdmaError rdmaError;
};
